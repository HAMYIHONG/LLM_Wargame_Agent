...
# import pickle
import random
# from .const import cond_to_text
# import numpy
...
class Map:
    ...
    ''' 
    def get_hex_text(self, hex):
        """
        获取坐标的信息
        地形，高程
        """
        hex_info = self.basic[hex // 100][hex % 100]
        return f"地形{cond_to_text(hex_info['cond'])}，高程{hex_info['elev']}"
    '''
        
    def get_hex_to_city_info(self, hex, city):
        """
        获取坐标点到夺控点的距离和通视
        """
        city_hex = city['coord']
        info = f"坐标{[hex%100, hex//100]}与{city['name']}{[city_hex%100, city_hex//100]}之间的距离为{self.get_distance(hex, city_hex)}，两点之间"
        if self.can_see(hex, city_hex, 0):
            info += "通视"
        else:
            info += "不通视"
        return info
